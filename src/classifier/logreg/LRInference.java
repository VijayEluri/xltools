...
//	public LogisticRegression lr;
	...
	public Prediction findBestLabel(Instance x, Model m) {
		...
		int[] predictions = new int[lr.labelFactory.all_labels.size()];
		double[] probs = new double[lr.labelFactory.all_labels.size()];
		...
		for (int t = 0; t < times; t++){
			...
			int L = lr.labelFactory.all_labels.size();
			int maxL = -1;
...
//				for (int i = 0; i < x.featureIndex.size(); i++){
				int startpoint = lr.featureFactory.all_features.size() * j;
...
//				}
		...
		return new Prediction(lr.labelFactory.all_labels.get(maxL), probs[maxL] / max);
		//		else
	...
	public ArrayList<Prediction> findPredictions(Instance x, Model m) {
		...
		for (int t = 0; t < times; t++){
			...
			for (int i = 0; i < L; i++){
				preds.add(new Prediction(lr.labelFactory.all_labels.get(i), /*Math.exp((scores.get(i) - max)) / part*/scores.get(i)));
			}
